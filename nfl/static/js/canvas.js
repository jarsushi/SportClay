var main =function(){
	...
	*/
	...
	var changeFormFieldsDownstream = function(node){
		...
		} else if(node.name=='filter' && node.fields){

			$('div#f' + node.id.substring(1,node.id.length) + ' table.filterTable tbody tr').remove('.filterFormField');

			for (i=1; i<=node.fields.length; i++){
				$('div#f' + node.id.substring(1,node.id.length) + ' table.filterTable').append("<tr class='filterFormField'><td class='fieldName'>" + String(node.fields[i-1]) + "</td><td class='include included' name='" + String(node.fields[i-1]) + "'>Included</td><td class='newName'>" + String(node.fields[i-1]) + "</td></tr>");
			}
		}
		return;
	}
